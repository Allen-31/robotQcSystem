import type { MenuNode } from '../shared/types/menu';

export const menuList: MenuNode[] = [
  {
    id: '2',
    name: 'menu.qualityInspection',
    code: 'qualityInspection',
    icon: 'CheckCircleOutlined',
    children: [
      {
        id: '2-1',
        name: 'menu.business',
        code: 'business',
        children: [
          {
            id: '2-1-1',
            name: 'menu.workstationManage',
            code: 'workstationManage',
            path: '/qualityInspection/workstationManage',
            permission: 'qualityInspection:workstationManage:view',
          },
          {
            id: '2-1-2',
            name: 'menu.workstationPositionManage',
            code: 'workstationPositionManage',
            path: '/qualityInspection/workstationPositionManage',
            permission: 'qualityInspection:workstationPositionManage:view',
          },
          {
            id: '2-1-3',
            name: 'menu.workOrderManage',
            code: 'workOrderManage',
            path: '/qualityInspection/workOrderManage',
            permission: 'qualityInspection:workOrderManage:view',
          },
          {
            id: '2-1-4',
            name: 'menu.qualityRecord',
            code: 'qualityRecord',
            path: '/qualityInspection/qualityRecord',
            permission: 'qualityInspection:qualityRecord:view',
          },
          {
            id: '2-1-5',
            name: 'menu.reinspectionRecord',
            code: 'reinspectionRecord',
            path: '/qualityInspection/reinspectionRecord',
            permission: 'qualityInspection:reinspectionRecord:view',
          },
        ],
        permission: 'qualityInspection:business:view',
      },
      {
        id: '2-2',
        name: 'menu.config',
        code: 'config',
        children: [
          {
            id: '2-2-1',
            name: 'menu.workstationConfig',
            code: 'workstationConfig',
            path: '/qualityInspection/workstationConfig',
            permission: 'qualityInspection:workstationConfig:view',
          },
          {
            id: '2-2-2',
            name: 'menu.workstationPositionConfig',
            code: 'workstationPositionConfig',
            path: '/qualityInspection/workstationPositionConfig',
            permission: 'qualityInspection:workstationPositionConfig:view',
          },
          {
            id: '2-2-3',
            name: 'menu.wireHarnessType',
            code: 'wireHarnessType',
            path: '/qualityInspection/wireHarnessType',
            permission: 'qualityInspection:wireHarnessType:view',
          },
          {
            id: '2-2-4',
            name: 'menu.terminalConfig',
            code: 'terminalConfig',
            path: '/qualityInspection/terminalConfig',
            permission: 'qualityInspection:terminalConfig:view',
          },
          {
            id: '2-2-5',
            name: 'menu.workshopConfig',
            code: 'workshopConfig',
            path: '/qualityInspection/workshopConfig',
            permission: 'qualityInspection:workshopConfig:view',
          },
        ],
        permission: 'qualityInspection:config:view',
      },
      {
        id: '2-3',
        name: 'menu.statistics',
        code: 'statistics',
        children: [
          {
            id: '2-3-1',
            name: 'menu.qualityStatistics',
            code: 'qualityStatistics',
            path: '/qualityInspection/qualityStatistics',
            permission: 'qualityInspection:qualityStatistics:view',
          },
          {
            id: '2-3-2',
            name: 'menu.qualityReport',
            code: 'qualityReport',
            path: '/qualityInspection/qualityReport',
            permission: 'qualityInspection:qualityReport:view',
          },
        ],
        permission: 'qualityInspection:statistics:view',
      },
    ],
    path: '/qualityInspection',
    permission: 'qualityInspection:view',
  },
  {
    id: '3',
    name: 'menu.deployConfig',
    code: 'deployConfig',
    icon: 'SettingOutlined',
    children: [
      {
        id: '3-1',
        name: 'menu.task',
        code: 'task',
        children: [
          {
            id: '3-1-1',
            name: 'menu.taskTemplate',
            code: 'taskTemplate',
            path: '/deployConfig/task/taskTemplate',
            permission: 'deployConfig:task:taskTemplate:view',
          },
          {
            id: '3-1-2',
            name: 'menu.actionTemplate',
            code: 'actionTemplate',
            path: '/deployConfig/task/actionTemplate',
            permission: 'deployConfig:task:actionTemplate:view',
          },
        ],
        permission: 'deployConfig:task:view',
      },
      {
        id: '3-2',
        name: 'menu.robot',
        code: 'robot',
        children: [
          {
            id: '3-2-1',
            name: 'menu.robotList',
            code: 'robotList',
            path: '/deployConfig/robot/robotList',
            permission: 'deployConfig:robot:robotList:view',
          },
          {
            id: '3-2-2',
            name: 'menu.robotType',
            code: 'robotType',
            path: '/deployConfig/robot/robotType',
            permission: 'deployConfig:robot:robotType:view',
          },
          {
            id: '3-2-3',
            name: 'menu.robotParts',
            code: 'robotParts',
            path: '/deployConfig/robot/robotParts',
            permission: 'deployConfig:robot:robotParts:view',
          },
          {
            id: '3-2-4',
            name: 'menu.robotGroup',
            code: 'robotGroup',
            path: '/deployConfig/robot/robotGroup',
            permission: 'deployConfig:robot:robotGroup:view',
          },
          {
            id: '3-2-5',
            name: 'menu.chargeStrategy',
            code: 'chargeStrategy',
            path: '/deployConfig/robot/chargeStrategy',
            permission: 'deployConfig:robot:chargeStrategy:view',
          },
          {
            id: '3-2-6',
            name: 'menu.homingStrategy',
            code: 'homingStrategy',
            path: '/deployConfig/robot/homingStrategy',
            permission: 'deployConfig:robot:homingStrategy:view',
          },
        ],
        permission: 'deployConfig:robot:view',
      },
      {
        id: '3-3',
        name: 'menu.scene',
        code: 'scene',
        children: [
          {
            id: '3-3-2',
            name: 'menu.mapManage',
            code: 'mapManage',
            path: '/deployConfig/scene/mapManage',
            permission: 'deployConfig:scene:mapManage:view',
          },
          {
            id: '3-3-3',
            name: 'menu.deviceManage',
            code: 'deviceManage',
            path: '/deployConfig/scene/deviceManage',
            permission: 'deployConfig:scene:deviceManage:view',
          },
        ],
        permission: 'deployConfig:scene:view',
      },
      {
        id: '3-4',
        name: 'menu.setting',
        code: 'setting',
        path: '/deployConfig/setting',
        permission: 'deployConfig:setting:view',
      },
      {
        id: '3-5',
        name: 'menu.user',
        code: 'user',
        children: [
          {
            id: '3-5-1',
            name: 'menu.userManage',
            code: 'userManage',
            path: '/deployConfig/user/userManage',
            permission: 'deployConfig:user:userManage:view',
          },
          {
            id: '3-5-2',
            name: 'menu.roleManage',
            code: 'roleManage',
            path: '/deployConfig/user/roleManage',
            permission: 'deployConfig:user:roleManage:view',
          },
        ],
        permission: 'deployConfig:user:view',
      },
    ],
    path: '/deployConfig',
    permission: 'deployConfig:view',
  },
  {
    id: '5',
    name: 'menu.operationMaintenance',
    code: 'operationMaintenance',
    icon: 'ToolOutlined',
    children: [
      {
        id: '5-1',
        name: 'menu.task',
        code: 'task',
        children: [
          {
            id: '5-1-1',
            name: 'menu.taskManage',
            code: 'taskManage',
            path: '/operationMaintenance/task/taskManage',
            permission: 'operationMaintenance:task:taskManage:view',
          },
        ],
        permission: 'operationMaintenance:task:view',
      },
      {
        id: '5-2',
        name: 'menu.robot',
        code: 'robot',
        children: [
          {
            id: '5-2-1',
            name: 'menu.robotManage',
            code: 'robotManage',
            path: '/operationMaintenance/robot/robotManage',
            permission: 'operationMaintenance:robot:robotManage:view',
          },
        ],
        permission: 'operationMaintenance:robot:view',
      },
      {
        id: '5-3',
        name: 'menu.notification',
        code: 'notification',
        children: [
          {
            id: '5-3-1',
            name: 'menu.exceptionNotification',
            code: 'exceptionNotification',
            path: '/operationMaintenance/notification/exceptionNotification',
            permission: 'operationMaintenance:notification:exceptionNotification:view',
          },
          {
            id: '5-3-2',
            name: 'menu.loginLog',
            code: 'loginLog',
            path: '/operationMaintenance/notification/loginLog',
            permission: 'operationMaintenance:notification:loginLog:view',
          },
          {
            id: '5-3-3',
            name: 'menu.operationLog',
            code: 'operationLog',
            path: '/operationMaintenance/notification/operationLog',
            permission: 'operationMaintenance:notification:operationLog:view',
          },
          {
            id: '5-3-4',
            name: 'menu.apiLog',
            code: 'apiLog',
            path: '/operationMaintenance/notification/apiLog',
            permission: 'operationMaintenance:notification:apiLog:view',
          },
        ],
        permission: 'operationMaintenance:notification:view',
      },
      {
        id: '5-4',
        name: 'menu.file',
        code: 'file',
        children: [
          {
            id: '5-4-1',
            name: 'menu.fileManage',
            code: 'fileManage',
            path: '/operationMaintenance/file/fileManage',
            permission: 'operationMaintenance:file:fileManage:view',
          },
        ],
        permission: 'operationMaintenance:file:view',
      },
      {
        id: '5-5',
        name: 'menu.upgrade',
        code: 'upgrade',
        children: [
          {
            id: '5-5-1',
            name: 'menu.packageManage',
            code: 'packageManage',
            path: '/operationMaintenance/upgrade/packageManage',
            permission: 'operationMaintenance:upgrade:packageManage:view',
          },
          {
            id: '5-5-2',
            name: 'menu.publishManage',
            code: 'publishManage',
            path: '/operationMaintenance/upgrade/publishManage',
            permission: 'operationMaintenance:upgrade:publishManage:view',
          },
        ],
        permission: 'operationMaintenance:upgrade:view',
      },
      {
        id: '5-6',
        name: 'menu.service',
        code: 'service',
        children: [
          {
            id: '5-6-1',
            name: 'menu.serviceManage',
            code: 'serviceManage',
            path: '/operationMaintenance/service/serviceManage',
            permission: 'operationMaintenance:service:serviceManage:view',
          },
        ],
        permission: 'operationMaintenance:service:view',
      },
    ],
    path: '/operationMaintenance',
    permission: 'operationMaintenance:view',
  },
  {
    id: '6',
    name: 'menu.dataStatistics',
    code: 'dataStatistics',
    icon: 'BarChartOutlined',
    children: [
      {
        id: '6-2',
        name: 'menu.deviceStatistics',
        code: 'deviceStatistics',
        path: '/dataStatistics/deviceStatistics',
        permission: 'dataStatistics:deviceStatistics:view',
      },
      {
        id: '6-3',
        name: 'menu.exceptionStatistics',
        code: 'exceptionStatistics',
        path: '/dataStatistics/exceptionStatistics',
        permission: 'dataStatistics:exceptionStatistics:view',
      },
    ],
    path: '/dataStatistics',
    permission: 'dataStatistics:view',
  },
];
