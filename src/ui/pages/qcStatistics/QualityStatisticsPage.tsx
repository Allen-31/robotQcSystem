...
export function QualityStatisticsPage() {
  ...
  const statisticsQuery = useQuery({
    queryKey: qualityAnalyticsQueryKeys.statistics({ period: 'month1', dimension: 'workshop' }),
    queryFn: () => getQualityStatisticsApi({ period: 'month1', dimension: 'workshop' }),
  });

  const apiRecords = useMemo<QualityStatRecord[] | null>(() => {
    const compare = statisticsQuery.data?.data?.compare ?? [];
    if (compare.length === 0) {
      return null;
    }
    return mapApiCompareToRecords(compare, 'workshop');
  }, [statisticsQuery.data?.data?.compare]);

  const statsLoading = statisticsQuery.isLoading || statisticsQuery.isFetching;
          ...
          <Space wrap>
            ...
            {period === 'custom' ? (
              <DatePicker.RangePicker
                ...
                style={{ width: 280 }}
                placeholder={locale === 'en-US' ? ['Start Date', 'End Date'] : ['Start Date', 'End Date']}
              />
            ) : null}
            <Select
              value={workshop}
        ...
        <Space direction="vertical" size={12} style={{ width: '100%' }}>
          ...
          {chartIsLine ? (
            ...
            <SimpleBarChart
              ...
              unit={metric === 'avgDurationMin' ? (locale === 'en-US' ? ' min' : ' min') : undefined}
            />
          )}
        </Space>
      </Card>
